/**
 * One-off reference-data migration: adds the POS and Vendor permissions that exist in the
 * code but are missing from a database seeded before those modules landed.
 *
 * Symptom it fixes: "Point of Sale" and "Vendors" missing from the admin sidebar, and the
 * POS/VENDORS groups absent from Roles & permissions entirely. The nav is permission-driven
 * (visibleNav in apps/web-admin/src/lib/roles.ts), and RoleAdminService.assertPermissionsExist
 * refuses to grant a key with no permission document — so this cannot be fixed from the UI.
 *
 ...
 *
 * This script is deliberately narrow: it upserts 4 permission documents and $addToSet's them
 * onto the roles the seeder intends. It never removes a permission, so hand-made role edits
 * (for example role:write deliberately unticked on Administrator) are preserved.
 *
 * Usage — reports what it WOULD do and changes nothing:
 *   MONGODB_URI="mongodb+srv://..." node scripts/add-pos-vendor-permissions.mjs
 * Apply for real:
 *   MONGODB_URI="mongodb+srv://..." node scripts/add-pos-vendor-permissions.mjs --apply
 *
...
const run = async () => {
  ...
  for (const [roleKey, keys] of Object.entries(GRANTS)) {
    ...
    console.log(`  + ${roleKey}: ${missing.join(', ')}  (${current.length} → ${current.length + missing.length})`);
    if (APPLY) {
      await roles.updateOne({ key: roleKey }, { $addToSet: { permissionKeys: { $each: missing } } });
    }
  }

  if (APPLY) {
    console.log(`\nDone. Permissions now: ${await permissions.countDocuments()}`);
